public class BinaryTree{
   ...
   static int search1(Node p, int key){
      ...
      else if(key<p.data) search1(p.left,key);
      else if(p.data<key) search1(p.right,key);
   ...
   static int search2(Node p, int key){
      if(p==null) return 0;
      if(p.data==key) return 1;
      else if(key<p.data) return search2(p.left,key);
      else if(p.data<key) return search2(p.right,key);
      return 0; // statement never reached
   } 
